...
export default function Pagination({ recipesPerPage, recipes, changePage, currentPage }) {
        ...
        <div className={s.pagination}>
            {pageNumbers &&
                pageNumbers.map(number => (     // Muestro todos los numeros de paginado que se crearon segun la cantidad total de recetas a mostrar.
                    <span key={number} onClick={() => changePage(number)} className={number === currentPage ? s.current_page : null}>{number}</span>   
                ))
            }
        </div>
    )
